...
int main(){
	...
	while(!bfs.empty()){
		...
		if(nj+1<=c && (a[ni][nj]=='R'||a[ni][nj]=='B') &&
			mark[ni][nj]<mark[ni][nj+1]){
		...
		if(ni+1<=r && (a[ni][nj]=='D'||a[ni][nj]=='B') &&
			mark[ni][nj]<mark[ni+1][nj]){
		...
		if(nj-1>=1 && (a[ni][nj-1]=='R'||a[ni][nj-1]=='B') &&
			mark[ni][nj]<mark[ni][nj-1]){
				if(mark[ni][nj-1]==mark[ni][nj]+1){
					printf("%d\n%d %d\n",mark[ni][nj]+1,ni,nj-1);
					return 0;
		...
		if(ni-1>=1 && (a[ni-1][nj]=='D'||a[ni-1][nj]=='B') &&
			mark[ni][nj]<mark[ni-1][nj]){
				if(mark[ni-1][nj]==mark[ni][nj]+1){
					printf("%d\n%d %d\n",mark[ni][nj]+1,ni-1,nj);
					return 0;
